...
if(any(is.na(schedule$team1_score[1:36]))) {
  ...
  group_stage_results <- sim_group_stage(dfs_group_stage)
...
df_r16 <- schedule %>% filter(ko_round == 'R16')
...
### R16
knockout_brackets <- 
  future_map_dfr(1:n_sims, ~df_r16, .id = 'sim_id') %>% 
  mutate('sim_id' = as.numeric(sim_id)) %>% 
...
knockout_brackets <-
  ...
  bind_cols(
    r16_results %>% 
      group_by(sim_id) %>% 
      reframe('tm1' = winner[c(1,3,5,7)],
              'tm2' = winner[c(2,4,6,8)]) %>% 
      ungroup() %>% 
      select(tm1, tm2)
  ) %>% 
  mutate('team1' = ifelse(is.na(team1), tm1, team1),
...
### Aggregate Results
qf_teams <- qf_results %>% pivot_longer(c('team1', 'team2')) %>% pull(value)
...
### Save Results
write_csv(df_stats, 'predictions/euro_sim_results.csv')
...
history <- 
  ...
  arrange(date)
write_csv(history, 'predictions/euro_history.csv')
...
write_csv(third_place, 'predictions/euro_third_place.csv')
